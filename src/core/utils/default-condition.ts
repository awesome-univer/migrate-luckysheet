...
import { INumberHighlightCell, CFRuleType, CFSubRuleType, CFNumberOperator, ITextHighlightCell, CFTextOperator, IUniqueValuesHighlightCell, IDuplicateValuesHighlightCell, IAverageHighlightCell, IRankHighlightCell } from "@univerjs/sheets-conditional-formatting";
...
function getRankInfo(conditionName: string | undefined, conditionValue: (string | number | object)[] | undefined) {
    let value = parseFloat((conditionValue as [string] || [])?.[0]);
    let isPercent = false;
    let isBottom = false;
    switch (conditionName) {
        case 'top10':
